      ...
      <el-form :model="article"
        ...
        <el-form-item>
          <el-button type="primary"
                     @click="onPublish(false)">{{ $route.query.id ? '修改' : '发表'}}</el-button>
    ...
    return {
      ...
      extensions: [
        ...
        new Image({
          // 默认会把图片生成 base64
          uploadRequest (file) {
            // 接口里的 Content-Type 设置为 multipart/form-data，只要给 data 一个 FormData 对象即可
        ...
        new Underline({ bubble: true, menubar: false }), // 在气泡菜单而不在菜单栏中渲染菜单按钮  下划线
        new Italic(), // 斜体
        new Strike(), // 删除线
  ...
  methods: {
    ...
    onPublish (draft = false) {
      this.$refs['publish-form'].validate(valid => {
        ...
        if (articleId) {
          // 修改文章
          updateArticles(articleId, this.article, draft).then(res => {
            this.$message({
